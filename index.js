...
if (text.includes(',ttp')){
  var teks = text.replace(/,ttp /, '')
    axios.get('https://st4rz.herokuapp.com/api/nulis?text')
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
...
axios.get(`https://mhankbarbar.herokuapp.com/api/nhentai?type=download&nuklir=${teks}&apiKey=SLpvUgOcMYwIx0pFeELt`).then((res) => {
	conn.sendMessage(id, '*[TUNGGU]*', MessageType.text)
