...
if (text.includes(",ttp")){
	...
	axios.get(`https://https://st4rz.herokuapp.com/api/ttp?kata=${teks}`).then((res) => {
    let hasil = `${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
if (text.includes(",weton")){
const teks = text.replace(/,weton /, "")
axios.get(`https://arugaz.herokuapp.com/api/getzodiak?nama=aruga&tgl-bln-thn`).then((res) => {
	conn.sendMessage(id, '*[TUNGGU]*', MessageType.text)
    let hasil = `*Tanggal :* _${res.data.tanggal}_\n*Bulan :* _${res.data.usia}_\n*Tahun :* _${res.data.tahun}_`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
