...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] BENTOL GANTENG`);
});
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => ig : @awsotoy`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => ig : @awsotoy`)
})
...
conn.sendMessage(id, '*Semoga Hari Ini Lebih Baik Dari Hari Kemarin.*' ,MessageType.text);
}
...
conn.sendMessage(id, '*https://github.com*' ,MessageType.text);
}
else if (text == 'Cuy'){
conn.sendMessage(id, 'Ya? Ketik ,menu' ,MessageType.text);
}
...
if (text.includes(",tingkatbucin")){
const teks = text.replace(/,tingkatbucin /, "")
axios.get(`https://arugaz.herokuapp.com/api/howbucins`).then((res) => {
	conn.sendMessage(id, '*[TUNGGU]*', MessageType.text)
    let hasil = `${res.data.desc}\n\nTingkat Bucin Kamu *${res.data.persen}%*`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://rest.farzain.com/api/jodoh.php?id=${teks}&apikey=ihcHwodYMFyEB4K6Q0Sh7jX1B`).then((res) => {
	let hasil = `*${res.data.result}*`
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbars.herokuapp.com/api/nhentai?type=download&nuklir=${teks}&apiKey=SLpvUgOcMYwIx0pFeELt`).then((res) => {
	conn.sendMessage(id, '*[TUNGGU]*', MessageType.text)
    let hasil = `*Judul* : ${res.data.title}\n *Pemeran* : ${res.data.artists}\n*Kode* : ${res.data.id}\n*Bahasa* : ${res.data.languages}\n*Tags* : ${res.data.tags}\n*Link Via Pdf* : ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
if (text.includes(",kbbi")){
const teks = text.replace(/,kbbi /, "")
axios.get(`https://mhankbarbars.herokuapp.com/api/kbbi?query=${teks}&lang=id&apiKey=SLpvUgOcMYwIx0pFeELt`).then((res) => {
	conn.sendMessage(id, '*[TUNGGU]*', MessageType.text)
    let hasil = `*Menurut Kamus Besar Bahasa Indonesia* :\n\n${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
if (text.includes(",tingkatgay")){
const teks = text.replace(/,tingkatgay /, "")
axios.get(`https://arugaz.herokuapp.com/api/howgay`).then((res) => {
	conn.sendMessage(id, '*[TUNGGU]*', MessageType.text)
    let hasil = `${res.data.desc}\n\nTingkat Gay Kamu *${res.data.persen}%*`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
