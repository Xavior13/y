...
conn.sendMessage(id, '*_Mau apa hayoh? wkwk. Mau tau jalanin fitur ini? Chat Owner : wa.me/6285770269605_*' ,MessageType.text);
}
else if (text == 'Mas'){
conn.sendMessage(id, 'Ya?, Ketik #help/#info/#donasi Contoh #help' ,MessageType.text);
}
...
if (text.includes('±woi')){
  var teks = text.replace(/±woi /, '')
    axios.get('https://nekos.life/api/v2/img/hentai')
    .then((res) => {
      imageToBase64(res.data.url)
        .then(
          (ress) => {
            conn.sendMessage(id, '*Jangan Dipake Macem Macem😠*', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
